...
class SendcloudConnector
{
    ...
    {
        if (!$this->isServicePointConfiguration($object, [self::SETTINGS_SERVICE_POINT])) {
            return;
        }
     ...
     * @param bool $reference Return the reference configuration
     */
     ...
     * @param bool $cache save the value to the class instance to avoid multiple DB hits
     *
    ...
    private function isServicePointConfiguration($object, $limit_names = null)
    {
        ...
        $configNames = !is_array($limit_names)
            ? [self::SETTINGS_SERVICE_POINT, self::SETTINGS_SELECTED_CARRIERS]
            : $limit_names;
