...
class SendcloudConnector
{
    ...
    public function activateServicePoints(Shop $shop, $object)
    {
        ...
        if (self::SETTINGS_SELECTED_CARRIERS === $object->name) {
            ...
            try {
                ...
                $object->delete();
                throw SendcloudServicePointException('Invalid carrier configuration: '. $e->getMessage());
            }
    ...
    private function removeLegacyCarrier() {
        ...
        if ($carrier->deleted || !$carrier->active) {
            SendcloudTools::log(sprintf('Sendcloud: last know carrier is deleted or disabled; ID=%d', $lastKnowLegacyID));
            ...
            $activeLegacyID = Db::getInstance()->getValue(sprintf(
                "SELECT `id_carrier` FROM `%s` WHERE external_module_name='%s' id_reference=%d AND deleted=0 AND active=1",
                pSQL(_DB_PREFIX_ . 'carrier'),
                pSQL($this->moduleName),
    ...
    private function getAllRegisteredCarrierCodes() {
        ...
        foreach ($data as $entry) {
            ...
            // Sendcloud carrier codes are sent in lowecase, but configurations are saved in all-caps.
            $code = strtolower($sanitized);
            $codes[] = $code;
        }
